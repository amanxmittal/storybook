...
//#region node_modules/jsdoc-type-pratt-parser/dist/index.js
var require_dist = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(global, factory) {
		...
		function getIdentifier(text) {
			...
			do {
				char = text[position];
				if (!identifierContinueRegex.test(char)) break;
				position++;
			} while (position < text.length);
			return text.slice(0, position);
		}
		...
		*/
		...
		class Parser {
			constructor(grammar, textOrLexer, baseParser) {
				...
				if (typeof textOrLexer === "string") this._lexer = Lexer.create(textOrLexer);
				else this._lexer = textOrLexer;
		...
		const nullableParslet = (parser, precedence, left) => {
			...
			if (!(left == null && type === "?" && !isQuestionMarkUnknownType(next) || left != null && type === "?")) return null;
			parser.consume("?");
			if (left == null) return {
		...
		const symbolParslet = composeParslet({
			...
			parseInfix: (parser, left) => {
				...
				if (!parser.consume(")")) {
					result.element = assertNumberOrVariadicNameResult(parser.parseIntermediateType(Precedence.SYMBOL));
				...
				parsePrefix: (parser) => {
					...
					if (!parser.consume("}")) {
						...
						while (true) {
							...
							if (field === void 0 && allowKeyTypes) field = parser.parseIntermediateType(Precedence.OBJECT);
							let optional = false;
							...
							if (field.type === "JsdocTypeNumber" || field.type === "JsdocTypeName" || field.type === "JsdocTypeStringValue") {
								let quote;
								if (field.type === "JsdocTypeStringValue") quote = field.meta.quote;
						...
						if (separator === "linebreak") result.meta.propertyIndent = "  ";
			...
			return composeParslet({
				...
				parseInfix: (parser, left) => {
					...
					if (left.type === "JsdocTypeNumber" || left.type === "JsdocTypeName" || left.type === "JsdocTypeStringValue" || isSquaredProperty(left)) {
						...
						let quote;
						if (left.type === "JsdocTypeStringValue") quote = left.meta.quote;
		...
		function createKeyValueParslet({ allowOptional, allowVariadic }) {
			return composeParslet({
				...
				parseInfix: (parser, left) => {
					...
					if (left.type !== "JsdocTypeName") throw new UnexpectedTypeError(left);
					parser.consume(":");
		...
		*/
		...
		function stringifyRules() {
			return {
				JsdocTypeParenthesis: (result, transform) => `(${result.element !== void 0 ? transform(result.element) : ""})`,
				...
				JsdocTypeFunction: (result, transform) => {
					...
					if (!result.arrow) {
						...
						if (result.returnType !== void 0) stringified += `: ${transform(result.returnType)}`;
						return stringified;
					} else {
						if (result.returnType === void 0) throw new Error("Arrow function needs a return type.");
						let stringified = `${result.typeParameters !== void 0 ? `<${(_a = result.typeParameters.map(transform).join(", ")) !== null && _a !== void 0 ? _a : ""}>` : ""}(${result.parameters.map(transform).join(", ")}) => ${transform(result.returnType)}`;
				...
				JsdocTypeVariadic: (result, transform) => result.meta.position === void 0 ? "..." : applyPosition(result.meta.position, transform(result.element), "..."),
				...
				JsdocTypeGeneric: (result, transform) => {
					...
					} else return `${transform(result.left)}${result.meta.dot ? "." : ""}<${result.infer === true ? "infer " : ""}${result.elements.map(transform).join(", ")}>`;
				},
				...
				JsdocTypeObjectField: (result, transform) => {
					...
					else text += transform(result.key);
					if (result.optional) text += "?";
					if (result.right === void 0) return text;
					else return text + `: ${transform(result.right)}`;
				},
				...
				JsdocTypeTypeParameter: (result, transform) => `${transform(result.name)}${result.constraint !== void 0 ? ` extends ${transform(result.constraint)}` : ""}${result.defaultValue !== void 0 ? ` = ${transform(result.defaultValue)}` : ""}`
			};
		}
		...
		const catharsisTransformRules = {
			...
			JsdocTypeFunction: (result, transform) => {
				...
				if (params.this !== void 0) transformed.this = transform(params.this);
				if (params.new !== void 0) transformed.new = transform(params.new);
				if (result.returnType !== void 0) transformed.result = transform(result.returnType);
				return transformed;
			},
			JsdocTypeGeneric: (result, transform) => ({
			...
			JsdocTypeObject: (result, transform) => {
				...
				else transformed.fields.push(transform(field));
				return transformed;
			},
			JsdocTypeObjectField: (result, transform) => {
				...
				return {
					...
					value: result.right === void 0 ? void 0 : transform(result.right)
				};
			},
			JsdocTypeJsdocObjectField: (result, transform) => ({
				type: "FieldType",
			...
			JsdocTypeNamePath: (result, transform) => {
				...
				let rightValue;
				if (result.right.type === "JsdocTypeSpecialNamePath") rightValue = transform(result.right).name;
			...
			JsdocTypeSymbol: (result) => {
				...
				if ((element === null || element === void 0 ? void 0 : element.type) === "JsdocTypeName") value += element.value;
				else if ((element === null || element === void 0 ? void 0 : element.type) === "JsdocTypeNumber") value += element.value.toString();
		...
		const jtpRules = {
			...
			JsdocTypeVariadic: (result, transform) => {
				...
				if (result.element !== void 0) transformed.value = transform(result.element);
				return transformed;
			},
			JsdocTypeName: (result) => ({
				type: "NAME",
			...
			JsdocTypeFunction: (result, transform) => {
				...
				const transformed = {
					...
					params: specialParams.params.map((param) => {
						...
						} else return transform(param);
					}),
					new: null,
					returns: null
				};
				if (specialParams.this !== void 0) transformed.this = transform(specialParams.this);
				else if (!result.arrow) transformed.this = null;
				if (specialParams.new !== void 0) transformed.new = transform(specialParams.new);
				if (result.returnType !== void 0) transformed.returns = transform(result.returnType);
				return transformed;
			},
			JsdocTypeGeneric: (result, transform) => {
				const transformed = {
			...
			JsdocTypeObject: (result, transform) => {
				const entries = [];
				for (const field of result.elements) if (field.type === "JsdocTypeObjectField" || field.type === "JsdocTypeJsdocObjectField") entries.push(transform(field));
				return {
					type: "RECORD",
					entries
				};
		...
		function identityTransformRules() {
			return {
				...
				JsdocTypeSymbol: (result, transform) => {
					...
					if (result.element !== void 0) transformed.element = transform(result.element);
					return transformed;
				},
				JsdocTypeOptional: (result, transform) => ({
					type: "JsdocTypeOptional",
				...
				JsdocTypeObjectField: (result, transform) => ({
					type: "JsdocTypeObjectField",
					key: result.key,
					right: result.right === void 0 ? void 0 : transform(result.right),
				...
				JsdocTypeFunction: (result, transform) => {
					...
					if (result.returnType !== void 0) transformed.returnType = transform(result.returnType);
					return transformed;
				},
				JsdocTypeKeyof: (result, transform) => ({
					type: "JsdocTypeKeyof",
				...
				JsdocTypeTypeParameter: (result, transform) => ({
					...
					constraint: result.constraint !== void 0 ? transform(result.constraint) : void 0,
					defaultValue: result.defaultValue !== void 0 ? transform(result.defaultValue) : void 0
				})
			};
		}
		const visitorKeys = {
		...
		function _traverse(node, parentNode, property, onEnter, onLeave) {
			onEnter === null || onEnter === void 0 || onEnter(node, parentNode, property);
			const keysToVisit = visitorKeys[node.type];
			for (const key of keysToVisit) {
				const value = node[key];
				if (value !== void 0) if (Array.isArray(value)) for (const element of value) _traverse(element, node, key, onEnter, onLeave);
				else _traverse(value, node, key, onEnter, onLeave);
			}
			onLeave === null || onLeave === void 0 || onLeave(node, parentNode, property);
		}
		/**
		...
		*/
		...
		exports$1.catharsisTransform = catharsisTransform;
		exports$1.identityTransformRules = identityTransformRules;
		exports$1.jtpTransform = jtpTransform;
		exports$1.parse = parse;
		exports$1.stringify = stringify;
		exports$1.stringifyRules = stringifyRules;
		exports$1.transform = transform;
		exports$1.traverse = traverse;
		exports$1.tryParse = tryParse;
