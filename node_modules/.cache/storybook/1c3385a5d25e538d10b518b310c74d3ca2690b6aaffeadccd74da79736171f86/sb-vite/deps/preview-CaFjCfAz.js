...
//#endregion
//#region node_modules/@storybook/addon-measure/dist/preview.mjs
...
function createCanvas() {
	...
	return setCanvasWidthAndHeight(canvas, context, {
		width,
		height
	}), canvas.style.position = "absolute", canvas.style.left = "0", canvas.style.top = "0", canvas.style.zIndex = "2147483647", canvas.style.pointerEvents = "none", import_global.global.document.body.appendChild(canvas), {
		canvas,
		context,
		width,
		height
...
function rescale() {
	invariant(state.canvas, "Canvas should exist in the state."), invariant(state.context, "Context should exist in the state."), setCanvasWidthAndHeight(state.canvas, state.context, {
		width: 0,
		height: 0
	});
	let { width, height } = getDocumentWidthAndHeight();
...
function configureText(context, text) {
	...
	let metrics = context.measureText(text), actualHeight = metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent;
	return {
		w: metrics.width + labelPadding * 2,
		h: actualHeight + labelPadding * 2
	};
}
...
function drawStack(context, measurements, stack, external) {
	let rects = [];
	stack.forEach((l, idx) => {
		rects[idx] = external && l.position === "center" ? drawFloatingLabel(context, measurements, l) : drawLabel(context, measurements, l, rects[idx - 1], external);
	});
}
...
function floatingAlignment(extremities) {
	let windowExtremities = {
		top: import_global.global.window.scrollY,
		bottom: import_global.global.window.scrollY + import_global.global.window.innerHeight,
		left: import_global.global.window.scrollX,
		right: import_global.global.window.scrollX + import_global.global.window.innerWidth
	}, distances = {
...
var decorators = [withMeasure], initialGlobals = { [PARAM_KEY]: !1 };
//#endregion
export { initialGlobals as n, decorators as t };

//# sourceMappingURL=preview-CaFjCfAz.js.map